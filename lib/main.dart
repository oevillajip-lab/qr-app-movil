...
class _MainScreenState extends State<MainScreen> {
  ...
  File? _logo;
  double _auraSize = 3.0; // Margen ajustable
  List<List<bool>> _logoAuraMap = []; 
  final GlobalKey _qrKey = GlobalKey();

  // --- PROCESAMIENTO INTELIGENTE DEL LOGO ---
  Future<void> _processLogo(File file) async {
    ...
    setState(() {
      ...
      _logoAuraMap = List.generate(res, (y) => List.generate(res, (x) {
        ...
        // Detecta si el píxel no es blanco/transparente (Forma real del logo)
        return pixel.luminance < 0.92 && pixel.a > 0.1;
      }));
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            _buildSection("1. Contenido", Column(children: [
            ...
            _buildSection("2. Estilo y Color QR", Column(children: [
              DropdownButtonFormField<String>(
                value: _estilo,
            ...
            _buildSection("3. Posicionamiento y Fondo", Column(children: [
            ...
            if (_logo != null) _buildSection("4. Ajuste de Aura", Column(children: [
               Text("Margen del Logo: ${_auraSize.toInt()}"),
               Slider(value: _auraSize, min: 1, max: 6, divisions: 5, onChanged: (v) => setState(() => _auraSize = v)),
            ])),
            ...
            ElevatedButton.icon(
              ...
              icon: const Icon(Icons.image), label: Text(_logo == null ? "SUBIR LOGO" : "LOGO CARGADO ✅"),
                  ...
                  child: Stack(alignment: Alignment.center, children: [
                    CustomPaint(
                      ...
                      painter: QrMasterPainter(
                        data: _c1.text.isEmpty ? "COMAGRO" : _c1.text, 
                        estilo: _estilo, auraMap: _logoAuraMap, auraSize: _auraSize,
            ...
            ElevatedButton(onPressed: () => _exportar(), child: const Text("GUARDAR QR"), style: ElevatedButton.styleFrom(backgroundColor: Colors.green, foregroundColor: Colors.white, minimumSize: const Size(double.infinity, 60))),
          ],
        ),
      ),
    );
  ...
  Widget _buildColorRow(String label, Color c1, Color c2, Function(Color) onC1, Function(Color) onC2, {bool isGrad = false}) {
    return Padding(padding: const EdgeInsets.symmetric(vertical: 8), child: Row(children: [
      ...
      _colorBtn(c1, onC1),
      if (isGrad) ...[const SizedBox(width: 10), _colorBtn(c2, onC2)],
    ]));
  }

  Widget _colorBtn(Color current, Function(Color) onSelect) {
    return GestureDetector(onTap: () => _showPalette(onSelect), child: CircleAvatar(backgroundColor: current, radius: 15, child: const Icon(Icons.colorize, size: 12, color: Colors.white)));
  }
  ...
  Future<void> _exportar() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("✅ Guardado en Galería")));
  }
}
    ...
    for (int r = 0; r < modules; r++) {
      for (int c = 0; c < modules; c++) {
        if (qrImage.isDark(r, c)) {
          // RECORTE DE AURA INTELIGENTE BASADO EN PÍXELES + MARGEN AJUSTABLE
