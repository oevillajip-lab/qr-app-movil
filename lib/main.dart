...
class _MainScreenState extends State<MainScreen> {
  ...
  Future<void> _processLogo(File file) async {
    ...
    setState(() {
      ...
      // MAPA DE COLISIÓN DE ALTA RESOLUCIÓN (100x100)
      int res = 100; 
      _collisionMap = List.generate(res, (y) => List.generate(res, (x) {
        ...
        // Detecta si es logo real (no fondo blanco de JPG y no transparente)
        ...
        child: Column(
          children: [
            ...
            if (_logo != null) _buildCard("4. Ajuste de Aura (Contorno)", Column(children: [
               ...
               Slider(value: _auraSize, min: 1, max: 8, divisions: 7, activeColor: Colors.black, onChanged: (v) => setState(() => _auraSize = v)),
            ])),
            ...
            ElevatedButton.icon(
              ...
              icon: const Icon(Icons.image_search), label: Text(_logo == null ? "CARGAR LOGO" : "LOGO CARGADO ✅"),
                  ...
                  child: isEmpty ? const Text("Esperando contenido...", style: TextStyle(color: Colors.grey)) : Stack(alignment: Alignment.center, children: [
                    CustomPaint(
                      ...
                      painter: QrMasterPainter(
                        data: finalData, estilo: _estilo, collisionMap: _collisionMap, auraSize: _auraSize,
  ...
  Widget _buildInputs() {
    switch (_qrType) {
      ...
      case "SMS (Mensaje)":
        return Column(children: [
          TextField(controller: _c1, decoration: const InputDecoration(hintText: "Número (+5959...)"), keyboardType: TextInputType.phone, onChanged: (v)=>setState((){})),
  ...
  Widget _colorBtn(Color current, Function(Color) onTap) {
    return GestureDetector(onTap: () => _showPalette(onTap), child: CircleAvatar(backgroundColor: current, radius: 20, child: Icon(Icons.colorize, size: 16, color: current == Colors.white ? Colors.black : Colors.white)));
  }

  void _showPalette(Function(Color) onSelect) {
    showDialog(context: context, builder: (ctx) => AlertDialog(content: Wrap(spacing: 12, runSpacing: 12, children: [Colors.black, Colors.white, Colors.red, Colors.blue, Colors.green, Colors.orange, Colors.purple, const Color(0xFF1565C0), Colors.grey].map((c) => GestureDetector(onTap: () { onSelect(c); Navigator.pop(ctx); }, child: CircleAvatar(backgroundColor: c, radius: 25))).toList())));
  }
...
// --- EL PINTOR MATEMÁTICO REAL (RECORTE DE SILUETA) ---
class QrMasterPainter extends CustomPainter {
    ...
    for (int r = 0; r < modules; r++) {
      for (int c = 0; c < modules; c++) {
        if (qrImage.isDark(r, c)) {
          // LÓGICA DE COLISIÓN POR PÍXEL
          if (collisionMap.isNotEmpty) {
            // Mapeamos la celda del QR al mapa de colisión (0 a 100)
            int mapX = (c * collisionMap.length ~/ modules);
            int mapY = (r * collisionMap.length ~/ modules);
            bool collision = false;
            int margin = auraSize.toInt();
            
            // Revisamos los vecinos según el margen (Aura ajustable)
            for (int dy = -margin; dy <= margin; dy++) {
              for (int dx = -margin; dx <= margin; dx++) {
                int ny = mapY + dy; int nx = mapX + dx;
                if (ny >= 0 && ny < collisionMap.length && nx >= 0 && nx < collisionMap.length) {
                  if (collisionMap[ny][nx]) { collision = true; break; }
                }
              }
              if (collision) break;
            }
            if (collision) continue; // Si choca con el logo real, no dibujamos el punto
          }
  ...
  void _drawEye(Canvas canvas, double x, double y, double t, Paint pE, Paint pI, bool circular) {
    double s = 7 * t;
    if (circular) {
