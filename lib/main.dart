...
// CORRECCIÓN: Agregada protección de Alignment Patterns para que el escáner no falle en Circular/Formas
bool _isStructural(int r, int c, int m) {
  if (_isEye(r, c, m)) return true;
  if (r == 6 || c == 6) return true; // timing patterns
  if (r <= 8 && c <= 8) return true; // format info
  if (r <= 8 && c >= m - 9) return true; // format info
  if (r >= m - 9 && c <= 8) return true; // version info
  if (m > 21 && r >= m - 10 && r <= m - 4 && c >= m - 10 && c <= m - 4) return true; // alignment pattern approx
  return false;
}
...
// ═══════════════════════════════════════════════════════════════════
// QR ADVANCED PAINTER (CORREGIDO CON ESCUDO VITAL _isVital)
// ═══════════════════════════════════════════════════════════════════
class QrAdvancedPainter extends CustomPainter {
  ...
  final double logoSize, auraSize; final bool customEyes;
  final Color qrC1, qrC2, eyeExt, eyeInt;
  const QrAdvancedPainter({required this.data, required this.estiloAvanzado, required this.qrMode, required this.qrC1, required this.qrC2, required this.logoImage, required this.outerMask, required this.logoSize, required this.auraSize, required this.qrDir, required this.customEyes, required this.eyeExt, required this.eyeInt});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    bool ok(int r, int c) {
      ...
      // LA CORRECCIÓN: Si es módulo vital, nunca lo recortamos.
      if (_isVital(r, c, m)) return true;
...
// ═══════════════════════════════════════════════════════════════════
// QR FUSION PAINTER (CORREGIDO PARA LEGIBILIDAD, Luma/Contraste Inverso)
// ═══════════════════════════════════════════════════════════════════
class QrFusionPainter extends CustomPainter {
  final String data, shape; final ui.Image? logoUiImage; final img_lib.Image? logoImage; final Color dominantColor;
  QrFusionPainter({required this.data, required this.logoUiImage, required this.logoImage, required this.shape, required this.dominantColor});

  @override
  void paint(Canvas canvas, Size size) {
    final qr = _buildQrImage(data); if (qr == null) return;
    final m = qr.moduleCount; final t = size.width / m;

    // Fondo completo de la imagen
    if (logoUiImage != null) {
        canvas.drawImageRect(
            logoUiImage!, 
            Rect.fromLTWH(0,0,logoUiImage!.width.toDouble(), logoUiImage!.height.toDouble()), 
            Rect.fromLTWH(0,0,size.width,size.height), 
            Paint()
        );
    }

    // Quiet zones (Cajas blancas sólidas tras los ojos para legibilidad absoluta)
    final qz = Paint()..color = Colors.white;
    canvas.drawRect(Rect.fromLTWH(-t, -t, 9*t, 9*t), qz);
    canvas.drawRect(Rect.fromLTWH((m-8)*t, -t, 9*t, 9*t), qz);
    canvas.drawRect(Rect.fromLTWH(-t, (m-8)*t, 9*t, 9*t), qz);

    // Módulos
    for (int r=0; r<m; r++) for (int c=0; c<m; c++) {
      if (_isEye(r, c, m)) continue;

      bool isDark = qr.isDark(r, c);
      double luma = 1.0; // Fondo por defecto claro
      
      if (logoImage != null) {
        final pixel = logoImage!.getPixel((c/m*logoImage!.width).floor().clamp(0, logoImage!.width-1), (r/m*logoImage!.height).floor().clamp(0, logoImage!.height-1));
        // Luma perceptual standard
        luma = (0.299 * pixel.r + 0.587 * pixel.g + 0.114 * pixel.b) / 255;
      }

      Paint? dotPaint;
      final double radius = t * 0.38; // Micro-punto para ver el fondo

      if (isDark) {
        // QR manda punto oscuro -> PINTAR NEGRO (no invertir datos)
        dotPaint = Paint()..color = Colors.black..isAntiAlias=true;
      } else {
        // QR manda espacio libre -> Si el fondo es muy oscuro, la cámara se confunde.
        // Forzamos un punto BLANCO para limpiar.
        if (luma < 0.45) {
      ...
      if (dotPaint != null) {
        if (shape == "Círculos") {
            canvas.drawCircle(Offset(c*t+t/2, r*t+t/2), radius, dotPaint);
        } else {
            canvas.drawRRect(RRect.fromRectAndRadius(
                Rect.fromCenter(center: Offset(c*t+t/2, r*t+t/2), width: radius*2, height: radius*2), 
                Radius.circular(t*0.15)), dotPaint);
        }
      }
    }

    // Ojos Negros Sólidos sobre las quiet zones
    final es = shape == "Círculos" ? EyeStyle.circ : EyeStyle.rect;
    _drawEye(canvas, 0, 0, t, Paint()..color=Colors.black, es);
    _drawEye(canvas, (m-7)*t, 0, t, Paint()..color=Colors.black, es);
    _drawEye(canvas, 0, (m-7)*t, t, Paint()..color=Colors.black, es);
  }
  @override bool shouldRepaint(CustomPainter old) => true;
}
