...
class _MainScreenState extends State<MainScreen>
  ...
  List<List<bool>> _maskFromAlpha(img_lib.Image src,
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
                        ...
                        return GestureDetector(
                          ...
                          child: AnimatedContainer(
                            ...
                            decoration: BoxDecoration(
                              ...
                              border: Border.all(
                                color:
                                    sel ? Colors.black : Colors.grey.shade300,
                                width: sel ? 2 : 1.5,
                              ),
                            ),
                            child: Text(
            ...
            Column(
              children: [
                ...
                if (_qrColorMode == "Degradado Custom")
                  DropdownButtonFormField<String>(
                    decoration:
                        const InputDecoration(labelText: "Dirección degradado"),
                    value: _qrGradDir,
          ...
          _card(
            "5. Logo${isShape ? ' y Forma' : ''}",
            _logoSection(effLogo, limited, isShape),
          ),
          _card(
            "6. Fondo",
            Column(
              children: [
                ...
                if (_bgMode != "Blanco (Default)" &&
                  ...
                  if (_bgMode == "Degradado")
                    DropdownButtonFormField<String>(
                      decoration:
                          const InputDecoration(labelText: "Dirección fondo"),
                      value: _bgGradDir,
                      items: ["Vertical", "Horizontal", "Diagonal"]
                          .map(
      ...
      children: [
        if (isShape) ...[
          ...
          const Padding(
            ...
            child: Text(
              "💡 Si tu logo es blanco, elige un fondo oscuro.",
              style: TextStyle(
                color: Colors.grey,
                fontSize: 12,
          ...
          if (_logoBytes != null) ...[
            ...
            Row(
              children: [
                ...
                const Spacer(),
                Text(
                  "${_auraSize.toStringAsFixed(1)} módulo(s)",
  ...
  Widget _qrPreview(
    ...
    final bgGrad =
        _bgMode == "Degradado" ? _getGrad(_bgC1, _bgC2, _bgGradDir) : null;
  ...
  Widget _actionButtons(bool isEmpty) => Row(
        children: [
          Expanded(
...
class StylePreviewPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int r = 0; r < m; r++) {
      for (int c = 0; c < m; c++) {
        ...
        if (style.contains("Gusano")) {
          ...
          if (ok(r, c + 1)) {
            lPath.moveTo(cx, cy);
            lPath.lineTo(cx + t, cy);
          }
          if (ok(r + 1, c)) {
            lPath.moveTo(cx, cy);
            lPath.lineTo(cx, cy + t);
          }
        } else if (style.contains("Barras")) {
          if (r == 0 || !ok(r - 1, c)) {
            int er = r;
            while (er + 1 < m && ok(er + 1, c)) {
              er++;
            }
          ...
          if (ok(r, c + 1) &&
              math.sqrt(math.pow(c + 1 - m / 2, 2) + math.pow(r - m / 2, 2)) <=
                  m / 2.1) {
            lPath.moveTo(cx, cy);
            lPath.lineTo(cx + t, cy);
          }
          ...
          if (ok(r + 1, c) &&
              math.sqrt(math.pow(c - m / 2, 2) + math.pow(r + 1 - m / 2, 2)) <=
                  m / 2.1) {
            lPath.moveTo(cx, cy);
            lPath.lineTo(cx, cy + t);
          }
    ...
    if (style.contains("Rombos") || style.contains("Diamant")) {
      es = EyeStyle.diamond;
    }
          ...
          for (double dy = 0.2; dy <= 0.8 && !hit; dy += 0.3) {
            for (double dx = 0.2; dx <= 0.8 && !hit; dx += 0.3) {
              ...
              if (nx >= ls && nx <= le && ny >= ls && ny <= le) {
                final px =
                    ((nx - ls) / lf * logoImage!.width).clamp(0, logoImage!.width - 1).toInt();
                final py =
                    ((ny - ls) / lf * logoImage!.height).clamp(0, logoImage!.height - 1).toInt();
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (isShape) {
      ...
      final double qrDarkRatio =
          qrDataCells == 0 ? 0.55 : (qrDarkCells / qrDataCells).clamp(0.35, 0.75);
        ...
        for (int rr = 0; rr < rows; rr++) {
          for (int cc = 0; cc < cols; cc++) {
            ...
            if (isDotsStyle) {
              ...
              canvas.drawRect(
                Rect.fromLTWH(x, y, decoT + 0.2, decoT + 0.2),
                solidPaint,
              );
            }
          }
