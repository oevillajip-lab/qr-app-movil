...
class _MainScreenState extends State<MainScreen> {
  ...
  Future<void> _processLogo(File file) async {
    ...
    setState(() {
      ...
      // Mapa de colisión 100x100 — ignora fondo blanco de JPG
      int res = 100;
      ...
      body: SingleChildScrollView(
        ...
        child: Column(children: [
          ...
          ElevatedButton.icon(
            ...
            label: Text(_logo == null ? "CARGAR LOGO" : "LOGO CARGADO ✅"),
                        ...
                        children: [
                          ...
                          if (_logo != null)
                            Container(
                              width: 65,
                              height: 65,
                              child: Image.file(_logo!, fit: BoxFit.contain),
                            ),
                        ],
                      ),
              ),
  ...
  Widget _colorBtn(Color current, Function(Color) onTap) {
    return GestureDetector(
      ...
      child: CircleAvatar(
        ...
        radius: 20,
        child: Icon(Icons.colorize, size: 16, color: current == Colors.white ? Colors.black : Colors.white),
      ),
    );
  }
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        content: Wrap(
          spacing: 12,
          runSpacing: 12,
          children: [Colors.black, Colors.white, Colors.red, Colors.blue, Colors.green, Colors.orange, Colors.purple, const Color(0xFF1565C0), Colors.grey]
...
// ================================================================
// QrMasterPainter — Integración real logo-QR con aura corregida
// ================================================================
class QrMasterPainter extends CustomPainter {
  ...
  bool _hitsLogo(int r, int c, int modules) {
    ...
    // Logo ocupa 65px de 270px → fracción del QR
    ...
    // Centro del módulo en espacio normalizado [0..1]
    final double nx = (c + 0.5) / modules;
    final double ny = (r + 0.5) / modules;

    // Coordenadas relativas dentro del logo [0..1]
    final double relX = (nx - logoStart) / logoFrac;
    final double relY = (ny - logoStart) / logoFrac;

    // Aura convertida a píxeles del mapa
    final double auraFrac = auraSize / modules;
    final double auraInMap = (auraFrac / logoFrac) * mapSize;
    final int auraInt = auraInMap.ceil().clamp(1, 10);
    ...
    for (int dy = -auraInt; dy <= auraInt; dy++) {
      for (int dx = -auraInt; dx <= auraInt; dx++) {
        final int cy = mapY + dy;
        final int cx = mapX + dx;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int r = 0; r < modules; r++) {
      for (int c = 0; c < modules; c++) {
        ...
        // Saltar ojos (se dibujan aparte)
        if (_isEyeModule(r, c, modules) && estilo != "Normal (Cuadrado)") continue;

        // Saltar módulos que colisionan con el logo + aura
  ...
  void _drawModule(Canvas canvas, Paint paint, double x, double y,
    ...
    if (estilo.contains("Gusano")) {
      ...
      if (r + 1 < modules && qrImage.isDark(r + 1, c)) {
        canvas.drawRect(
          Rect.fromLTWH(x + tileSize * 0.1, y + tileSize / 2, tileSize * 0.8, tileSize),
          paint,
        );
      }
